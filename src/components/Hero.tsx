...
const t = (delay: number): Transition => ({ duration: 0.75, ease: EASE, delay })
...
export default function Hero({ loaded }: { loaded: boolean }) {
  ...
  return (
    <header ref={heroRef} style={{ position: 'relative', minHeight: '100vh', padding: '120px 0 80px', display: 'flex', flexDirection: 'column', justifyContent: 'center', overflow: 'hidden' }}>
      {/* grid bg with parallax */}
      <motion.div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', backgroundImage: 'linear-gradient(to right, var(--line) 1px, transparent 1px), linear-gradient(to bottom, var(--line) 1px, transparent 1px)', backgroundSize: '96px 96px', maskImage: 'radial-gradient(ellipse 70% 70% at 50% 40%, #000 30%, transparent 80%)', WebkitMaskImage: 'radial-gradient(ellipse 70% 70% at 50% 40%, #000 30%, transparent 80%)', y: bgParallax, willChange: 'transform' }} />

      {/* main content with parallax */}
      <motion.div style={{ maxWidth: 1440, margin: '0 auto', padding: '0 56px', width: '100%', y: contentParallax, opacity: contentOpacity, willChange: 'transform' }}>
        {/* meta row */}
        ...
        <motion.div initial={init} animate={vis} transition={t(0.7)}
          style={{ marginTop: 36, display: 'flex', flexDirection: 'column', gap: 6, fontSize: 'clamp(20px, 2.4vw, 30px)', fontWeight: 400, letterSpacing: '-0.01em' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 14 }}>
            ...
            <span>& <span style={{ fontFamily: 'var(--serif)', fontStyle: 'italic', color: 'var(--accent)' }}>AI Enthusiast</span></span>
          </div>
        </motion.div>

        {/* subhead */}
        <motion.p initial={init} animate={vis} transition={t(0.75)}
          style={{ marginTop: 28, maxWidth: 560, fontSize: 15, lineHeight: 1.65, color: 'var(--ink-dim)' }}>
        ...
        <motion.div initial={init} animate={vis} transition={t(0.8)}
          ...
          <a href="#work" data-h=""
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '14px 22px', borderRadius: 980, background: 'var(--accent)', color: '#1a0d04', border: '1px solid var(--accent)', fontFamily: 'var(--sans)', fontSize: 13, letterSpacing: '.02em', textDecoration: 'none', cursor: 'none', transition: 'filter 0.3s var(--ease), transform 0.3s var(--ease)', willChange: 'transform' }}
          ...
          <a href="#contact" data-h=""
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 10, padding: '14px 22px', borderRadius: 980, background: 'transparent', color: 'var(--ink)', border: '1px solid var(--line-strong)', fontFamily: 'var(--sans)', fontSize: 13, letterSpacing: '.02em', textDecoration: 'none', cursor: 'none', transition: 'background 0.3s var(--ease), color 0.3s var(--ease), border-color 0.3s var(--ease), transform 0.3s var(--ease)', willChange: 'transform' }}
